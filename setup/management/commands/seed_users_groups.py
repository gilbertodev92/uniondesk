...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        ct, _ = ContentType.objects.get_or_create(
            app_label="setup", model="modulo"
        )

        perms = {}
        for m in modulos:
            p, _ = Permission.objects.get_or_create(
                codename=m.lower().replace(" ", "_").replace("-", "_"),
        ...
        for nome, grupos_user in usuarios.items():
            ...
            if created:
                u.set_password("123456")  # senha inicial (mude depois)
                u.is_active = True
                u.save()
            u.groups.set([grupo_objs[g] for g in grupos_user])

        self.stdout.write(self.style.SUCCESS("✅ Usuários, grupos e permissões criados com sucesso!"))
